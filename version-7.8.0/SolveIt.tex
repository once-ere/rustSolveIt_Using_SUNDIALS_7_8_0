...
Checking your build: \code{cargo test -{}-workspace} --- 568 tests should
pass.
...
\code{cvodes\_rs} & CVODE plus forward and adjoint sensitivity analysis & not yet\\
\code{ida\_rs}    & differential-algebraic systems $F(t,y,\dot y)=0$ & not yet\\
\code{idas\_rs}   & IDA plus sensitivity analysis & not yet\\
\code{kinsol\_rs} & nonlinear algebraic systems, Anderson acceleration & not yet\\
\end{tabular}
\end{center}
...
\section{Sixteen worked examples}\label{sec:examples}
...
568 tests: 40 library, 19 collision, 9 conservation, 109 language, 92
...
\code{scripts/solveit/} & the sixteen examples in \S\ref{sec:examples}\\
