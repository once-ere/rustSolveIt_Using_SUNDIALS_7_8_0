...
Checking your build: \code{cargo test -{}-workspace} --- 603 tests should
pass.
...
\textbf{Two limits worth knowing.} Orientation joints are integrated
\emph{from rest} --- a body already turning at $t_0$ is refused by name
--- and they carry a tolerance floor of $\texttt{rtol}=10^{-6}$, because
the index-2 system has an accuracy ceiling no tolerance can push past.
Both are refusals rather than guesses: a run that looks like it worked
and is not the mechanism you described is the worst possible outcome.
...
603 tests: 40 library, 19 collision, 9 conservation, 109 language, 92
