...
\subsection{The four families you are not using yet}

\begin{center}
\begin{tabular}{ll}
\textbf{crate} & \textbf{solves}\\\hline
\code{cvode\_rs}  & ODE initial-value problems (Adams / BDF) --- \emph{used}\\
\code{arkode\_rs} & Runge--Kutta: explicit, implicit, IMEX, multirate, symplectic --- \emph{used}\\
\code{cvodes\_rs} & CVODE plus forward and adjoint sensitivity analysis\\
\code{ida\_rs}    & differential-algebraic systems $F(t,y,\dot y)=0$\\
\code{idas\_rs}   & IDA plus sensitivity analysis\\
\code{kinsol\_rs} & nonlinear algebraic systems, with Anderson acceleration\\
\end{tabular}
\end{center}

They build, they carry their own examples, and each of those examples is
diffed against the upstream C reference output --- see
\code{sundials\_rs/VERIFICATION.md}. Nothing in the command language
reaches them today. If you add a constrained mechanism (a pendulum
written as a rod constraint rather than a force), \code{ida\_rs} is
where it would go.
...
\subsection{The three shipped recordings}
