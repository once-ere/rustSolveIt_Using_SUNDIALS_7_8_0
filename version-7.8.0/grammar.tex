...
\subsection{The nine shipped recordings}
...
\subsection{A hinge hands you a conservation law}
...
& \code{spinning\_top} & \code{gyroscope\_gimbal}\\\hline
mount & one \kw{ball} at the tip & three \kw{hinge}s, perpendicular
axes\\
rows & 3 on 6 freedoms & 15 on 18 freedoms\\
centre of mass & a distance $r = 0.6$ from the pivot & \textbf{on} the
pivot\\
gravity & lever arm $r$, so a torque & lever arm zero, so none\\
what happens & steady precession at $Mgr/(I_3\omega_3)$ & no
gravitational precession at all\\
\end{tabular}
\end{center}

The gimbal scene keeps the same \code{uniform\_gravity} as the top, and
it does nothing. That is not a setting to notice; it is the point. A
...
vertical, and the total $L\cdot\hat y = 0.6314$ is conserved to
$6.4\times10^{-15}$. The innermost hinge's axis \emph{is} the rotor's
...
that origin than $7\times10^{-35}$. The rings only \emph{look} nested.
...
$11.15^\circ$ from where it began, and the rotation reappears as a
\textbf{tilt} --- the rotor's axis swinging up to $20.06^\circ$ away, at
