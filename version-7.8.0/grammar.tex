...
\textbf{Two limits, both refused by name.} Orientation joints are
integrated \emph{from rest}: a body the mechanism sets turning is fine, a
body already turning at $t_0$ is refused. And they carry a tolerance
floor of $\texttt{rtol} = 10^{-6}$, because the differential-algebraic
system a hinge produces is \emph{index 2} and has an accuracy ceiling no
tolerance can push past. \kw{run} reports when the floor was applied.
